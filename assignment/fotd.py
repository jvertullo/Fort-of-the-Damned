...
DISPLAYSURF = display.set_mode((0, 0), FULLSCREEN)
...
def update():
    ...
    if keyboard.escape:
        quit()
    if game == 1:
        if (playr.collidepoint(mouse.get_pos())) and (mousedown):
            BACKGROUND_IMAGE = "bwip"
            game = 2
            mousedown = False
        if (helpra.collidepoint(mouse.get_pos())) and (mousedown):
            game = 5
            mousedown = False
    elif game == 2:
        ll = 800 - bhealth * 4
        ...
        BACKGROUND_IMAGE = "start"
        if (playar.collidepoint(mouse.get_pos())) and (mousedown):
            BACKGROUND_IMAGE = "bwip"
            varset()
            game = 2
            mousedown = False
        ...
        BACKGROUND_IMAGE = "start"
        if (back.collidepoint(mouse.get_pos())) and (mousedown):
            BACKGROUND_IMAGE = "start"
            game = 1
            mousedown = False
    mousedown = False
